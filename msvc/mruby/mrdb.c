...
parse_args(mrb_state *mrb, int argc, wchar_t **argv, struct _args *args)
{
	...
	*args = args_zero;
		...
		case L'd':
			...
			append_srcpath:
				if (!args->srcpath) {
					...
					srcpathlen = wcslen(args->srcpath);
					itemlen = wcslen(item);
					args->srcpath =
						(wchar_t *)mrb_realloc(mrb, args->srcpath, (srcpathlen + itemlen + 2) * sizeof(wchar_t));
					args->srcpath[srcpathlen] = L'\n';
					memcpy(args->srcpath + srcpathlen + 1, item, itemlen + 1);
				}
			}
			else {
				fprintf(stdout, "%ls: No path specified for -d\n", *origargv);
				return EXIT_SUCCESS;
			}
			break;
		case L'-':
	...
	if (args->rfp == NULL) {
		if (*argv == NULL) {
			fprintf(stdout, "%ls: Program file not specified.\n", *origargv);
			return EXIT_FAILURE;
		}
		else {
...
get_command(mrb_state *mrb, mrdb_state *mrdb)
{
	...
	if (i == 0 && feof(stdin)) {
		clearerr(stdin);
		strcpy_s(mrdb->command, MAX_COMMAND_LINE + 1, "quit");
		i += sizeof("quit") - 1;
	}

	if (i == MAX_COMMAND_LINE) {
		for (; (c = getc(stdin)) != EOF && c != '\n'; i++);
	}

	if (i > MAX_COMMAND_LINE) {
		fprintf(stdout, "command line too long.\n");
...
print_info_stopped_break(mrb_state *mrb, mrdb_state *mrdb)
{
	...
	if (ret == 0) {
		...
		case MRB_DEBUG_BPTYPE_METHOD:
			...
			if (class_name == NULL) {
				fprintf(stdout, "Breakpoint %d, %s\n", bp.bpno, method_name);
			}
			else {
				fprintf(stdout, "Breakpoint %d, %s:%s\n", bp.bpno, class_name, method_name);
			}
			if (mrdb->dbg->isCfunc) {
				fprintf(stdout, "Stopped before calling the C function.\n");
			}
			break;
		default:
			break;
...
get_and_parse_command(mrb_state *mrb, mrdb_state *mrdb)
{
	...
	while (!cmd) {
		for (p = NULL; !p || *p == '\0'; ) {
			fprintf(stdout, "(%s:%d) ", mrdb->dbg->prvfile, mrdb->dbg->prvline);
			fflush(stdout);
...
mrdb_main(int argc, wchar_t **argv)
{
...
l_restart:
	...
	if (args.mrbfile) { /* .mrb */
		...
		args.fname_utf8 = mrb_utf8_from_wchar(args.fname, -1);
		mrbc_filename(mrb, cc, args.fname_utf8);
