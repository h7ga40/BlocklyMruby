...
namespace BlocklyMruby
{
	...
	public partial class MrbParser : MrbParser.yyInput, MrbParser.yyConsoleOut
	{
		...
		static MrbParser.yyDebugOut debug;
			...
			for (int i = idx; i < end; i++) {
				...
				if (t == 0) {
					...
					if (c == t) {
						switch (t) {
						case 2:
							str += Char.ConvertFromUtf32(((temp[0] & 0x1F) << 6) | (temp[1] & 0x3F));
							break;
						case 3:
							str += Char.ConvertFromUtf32(((temp[0] & 0x0F) << 12) | ((temp[1] & 0x3F) << 6) | (temp[2] & 0x3F));
							break;
						case 4:
							str += Char.ConvertFromUtf32(((temp[0] & 0x07) << 18) | ((temp[1] & 0x3F) << 12) | ((temp[2] & 0x3F) << 6) | (temp[3] & 0x3F));
							break;
						case 5:
							str += Char.ConvertFromUtf32(((temp[0] & 0x03) << 24) | ((temp[1] & 0x3F) << 18) | ((temp[2] & 0x3F) << 12) | ((temp[3] & 0x3F) << 6) | (temp[4] & 0x3F));
							break;
						case 6:
							str += Char.ConvertFromUtf32(((temp[0] & 0x01) << 30) | ((temp[1] & 0x3F) << 24) | ((temp[2] & 0x3F) << 18) | ((temp[3] & 0x3F) << 12) | ((temp[4] & 0x3F) << 6) | (temp[5] & 0x3F));
							break;
						}
						t = 0;
						c = 0;
		...
		static int scan_hex(int[] start, int len, ref int retlen)
		{
		...
		private void mrb_parser_parse()
		{
			...
			try {
				...
				yyParse(this, verbose ? debug : null);
				if (this.tree == null) {
					this.tree = new_nil();
				}
#if false
				if (c != null && c.dump_result) {
					mrb_parser_dump(this.mrb, this.tree, 0);
				}
#endif
			}
			catch (Exception) {
		...
		void dump_recur(node tree, int offset)
		{
			while (tree != null) {
				parser_dump((node)tree.car, offset);
				tree = (node)tree.cdr;
			}
		}

		void parser_dump(node tree_, int offset)
		{
			node_type nodetype;

			if (tree_ == null) return;
			again:
			dump_prefix(tree_, offset);
			nodetype = (node_type)tree_.car;
			switch (nodetype) {
			case node_type.NODE_BEGIN: {
					var ast = (begin_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_BEGIN:\n");
					dump_recur((node)tree, offset + 1);
				}
				break;
			case node_type.NODE_RESCUE: {
					var ast = (rescue_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_RESCUE:\n");
					if (tree.car != null) {
						dump_prefix(tree, offset + 1);
						printf("body:\n");
						parser_dump((node)tree.car, offset + 2);
					}
					tree = (node)tree.cdr;
					if (tree.car != null) {
						node n2 = (node)tree.car;
						...
						while (n2 != null) {
							...
							if (n3.car != null) {
								...
								dump_recur((node)n3.car, offset + 3);
							}
							if (((node)n3.cdr).car != null) {
								dump_prefix(n3, offset + 2);
								printf("exc_var:\n");
								parser_dump((node)((node)n3.cdr).car, offset + 3);
							}
							if (((node)((node)n3.cdr).cdr).car != null) {
								dump_prefix(n3, offset + 2);
								printf("rescue body:\n");
								parser_dump((node)((node)((node)n3.cdr).cdr).car, offset + 3);
							}
							n2 = (node)n2.cdr;
						}
					}
				...
				break;
			case node_type.NODE_ENSURE: {
					var ast = (ensure_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_ENSURE:\n");
					dump_prefix(tree, offset + 1);
					printf("body:\n");
					...
					dump_prefix(tree, offset + 1);
					printf("ensure:\n");
					parser_dump((node)((node)tree.cdr).cdr, offset + 2);
				}
				break;
			case node_type.NODE_LAMBDA: {
					var ast = (lambda_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_BLOCK:\n");
					goto block;
				}
			case node_type.NODE_BLOCK:
				block:
				{
					var ast = (block_node)tree_;
					printf("NODE_BLOCK:\n");
					var tree = (node)ast.cdr;
					if (tree.car != null) {
						node n = (node)tree.car;

						if (n.car != null) {
							dump_prefix(n, offset + 1);
							printf("mandatory args:\n");
							dump_recur((node)n.car, offset + 2);
						}
						n = (node)n.cdr;
						if (n.car != null) {
							dump_prefix(n, offset + 1);
							printf("optional args:\n");
							{
								...
								while (n2 != null) {
									dump_prefix(n2, offset + 2);
									printf("%s=", sym2name((mrb_sym)((node)n2.car).car));
									parser_dump((node)((node)n2.car).cdr, 0);
									n2 = (node)n2.cdr;
								}
							}
						}
					...
					dump_prefix(tree, offset + 1);
					printf("body:\n");
					parser_dump((node)((node)tree.cdr).car, offset + 2);
				}
				break;
			case node_type.NODE_IF: {
					var ast = (if_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_IF:\n");
					dump_prefix(tree, offset + 1);
					printf("cond:\n");
					...
					printf("then:\n");
					parser_dump((node)((node)tree.cdr).car, offset + 2);
					if (((node)((node)tree.cdr).cdr).car != null) {
						dump_prefix(tree, offset + 1);
						printf("else:\n");
						parser_dump((node)((node)((node)tree.cdr).cdr).car, offset + 2);
					}
				}
				break;
			case node_type.NODE_AND: {
					var ast = (and_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_AND:\n");
					parser_dump((node)tree.car, offset + 1);
					parser_dump((node)tree.cdr, offset + 1);
				}
				break;
			case node_type.NODE_OR: {
					var ast = (or_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_OR:\n");
					parser_dump((node)tree.car, offset + 1);
					parser_dump((node)tree.cdr, offset + 1);
				...
				break;
			case node_type.NODE_CASE: {
					var ast = (case_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_CASE:\n");
					if (tree.car != null) {
						parser_dump((node)tree.car, offset + 1);
					...
					while (tree != null) {
						dump_prefix(tree, offset + 1);
						printf("case:\n");
						dump_recur((node)((node)tree.car).car, offset + 2);
						dump_prefix(tree, offset + 1);
						printf("body:\n");
						parser_dump((node)((node)tree.car).cdr, offset + 2);
						tree = (node)tree.cdr;
					}
				}
				break;
			case node_type.NODE_WHILE: {
					var ast = (while_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_WHILE:\n");
					dump_prefix(tree, offset + 1);
					printf("cond:\n");
					...
					dump_prefix(tree, offset + 1);
					printf("body:\n");
					parser_dump((node)tree.cdr, offset + 2);
				}
				break;
			case node_type.NODE_UNTIL: {
					var ast = (until_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_UNTIL:\n");
					dump_prefix(tree, offset + 1);
					printf("cond:\n");
				...
				break;
			case node_type.NODE_FOR: {
					var ast = (for_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_FOR:\n");
					dump_prefix(tree, offset + 1);
					printf("var:\n");
					{
						...
						if (n2.car != null) {
							dump_prefix(n2, offset + 2);
							printf("pre:\n");
							dump_recur((node)n2.car, offset + 3);
						}
						n2 = (node)n2.cdr;
						if (n2 != null) {
							if (n2.car != null) {
								dump_prefix(n2, offset + 2);
								printf("rest:\n");
								parser_dump((node)n2.car, offset + 3);
							}
							n2 = (node)n2.cdr;
							if (n2 != null) {
								if (n2.car != null) {
				...
				break;
			case node_type.NODE_SCOPE: {
					var ast = (scope_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_SCOPE:\n");
					{
						node n2 = (node)tree.car;
						bool first_lval = true;
						...
						if (n2 != null && (n2.car != null || n2.cdr != null)) {
							...
							while (n2 != null) {
								if (n2.car != null) {
									...
									printf("%s", sym2name((mrb_sym)n2.car));
									first_lval = false;
								}
								n2 = (node)n2.cdr;
							}
			...
			case node_type.NODE_FCALL: {
					var ast = (fcall_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_FCALL:\n");
					parser_dump((node)tree.car, offset + 1);
					dump_prefix(tree, offset + 1);
					printf("method='%s' (%d)\n",
						sym2name((mrb_sym)((node)tree.cdr).car),
						(int)((node)tree.cdr).car);
					tree = (node)((node)((node)tree.cdr).cdr).car;
					if (tree != null) {
						dump_prefix(tree, offset + 1);
						printf("args:\n");
						dump_recur((node)tree.car, offset + 2);
						if (tree.cdr != null) {
							dump_prefix(tree, offset + 1);
							printf("block:\n");
							parser_dump((node)tree.cdr, offset + 2);
				...
				break;
			case node_type.NODE_CALL:
			case node_type.NODE_SCALL: {
					var ast = (call_node)tree_;
					var tree = (node)ast.cdr;
					if (nodetype == node_type.NODE_CALL)
						printf("NODE_CALL:\n");
					else
						printf("NODE_CALL:\n");
					parser_dump((node)tree.car, offset + 1);
				...
				break;
			case node_type.NODE_DOT2: {
					var ast = (dot2_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_DOT2:\n");
					parser_dump((node)tree.car, offset + 1);
					parser_dump((node)tree.cdr, offset + 1);
				...
				break;
			case node_type.NODE_DOT3: {
					var ast = (dot3_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_DOT3:\n");
					parser_dump((node)tree.car, offset + 1);
					parser_dump((node)tree.cdr, offset + 1);
				...
				break;
			case node_type.NODE_COLON2: {
					var ast = (colon2_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_COLON2:\n");
					parser_dump((node)tree.car, offset + 1);
					dump_prefix(tree, offset + 1);
					printf("::%s\n", sym2name((mrb_sym)tree.cdr));
				}
				break;
			case node_type.NODE_COLON3: {
					var ast = (colon3_node)tree_;
					var tree = (mrb_sym)ast.cdr;
					printf("NODE_COLON3: ::%s\n", sym2name(tree));
				}
				break;
			case node_type.NODE_ARRAY: {
					var ast = (array_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_ARRAY:\n");
					dump_recur((node)tree, offset + 1);
				}
				break;
			case node_type.NODE_HASH: {
					var ast = (hash_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_HASH:\n");
					while (tree != null) {
						dump_prefix(tree, offset + 1);
						printf("key:\n");
						parser_dump((node)((node)tree.car).car, offset + 2);
						dump_prefix(tree, offset + 1);
						printf("value:\n");
						parser_dump((node)((node)tree.car).cdr, offset + 2);
						tree = (hash_node)tree.cdr;
					}
				}
				break;
			case node_type.NODE_SPLAT: {
					var ast = (splat_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_SPLAT:\n");
					parser_dump(tree, offset + 1);
				}
				break;
			case node_type.NODE_ASGN: {
					var ast = (asgn_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_ASGN:\n");
					dump_prefix(tree, offset + 1);
					printf("lhs:\n");
				...
				break;
			case node_type.NODE_MASGN: {
					var ast = (masgn_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_MASGN:\n");
					dump_prefix(tree, offset + 1);
					printf("mlhs:\n");
					{
						...
						if (n2 != null) {
							if (n2.car != null) {
								dump_prefix(n2, offset + 2);
								printf("rest:\n");
								if ((MrbTokens)n2.car == (MrbTokens)(-1)) {
									dump_prefix(n2, offset + 2);
									printf("(empty)\n");
								}
								else {
				...
				break;
			case node_type.NODE_OP_ASGN: {
					var ast = (op_asgn_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_OP_ASGN:\n");
					dump_prefix(tree, offset + 1);
					printf("lhs:\n");
					...
					tree = (node)tree.cdr;
					dump_prefix(tree, offset + 1);
					printf("op='%s' (%d)\n", sym2name((mrb_sym)tree.car), (int)tree.car);
					tree = (node)tree.cdr;
					parser_dump((node)tree.car, offset + 1);
				}
				break;
			case node_type.NODE_SUPER: {
					var ast = (super_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_SUPER:\n");
					if (tree != null) {
						dump_prefix(tree, offset + 1);
				...
				break;
			case node_type.NODE_ZSUPER: {
					var ast = (zsuper_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_ZSUPER\n");
				}
				break;
			case node_type.NODE_RETURN: {
					var ast = (return_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_RETURN:\n");
					parser_dump(tree, offset + 1);
				}
				break;
			case node_type.NODE_YIELD: {
					var ast = (yield_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_YIELD:\n");
					dump_recur((node)tree, offset + 1);
				}
			...
			case node_type.NODE_NEXT: {
				...
				break;
			case node_type.NODE_REDO: {
					var ast = (redo_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_REDO\n");
				}
				break;
			case node_type.NODE_RETRY: {
					var ast = (retry_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_RETRY\n");
				}
				break;
			case node_type.NODE_LVAR: {
					var ast = (lvar_node)tree_;
					var tree = (mrb_sym)ast.cdr;
					printf("NODE_LVAR %s\n", sym2name(tree));
				}
				break;
			case node_type.NODE_GVAR: {
					var ast = (gvar_node)tree_;
					var tree = (mrb_sym)ast.cdr;
					printf("NODE_GVAR %s\n", sym2name(tree));
				}
				break;
			case node_type.NODE_IVAR: {
					var ast = (ivar_node)tree_;
					var tree = (mrb_sym)ast.cdr;
					printf("NODE_IVAR %s\n", sym2name(tree));
				}
				break;
			case node_type.NODE_CVAR: {
					var ast = (cvar_node)tree_;
					var tree = (mrb_sym)ast.cdr;
					printf("NODE_CVAR %s\n", sym2name(tree));
				}
				break;
			case node_type.NODE_CONST: {
					var ast = (const_node)tree_;
					var tree = (mrb_sym)ast.cdr;
					printf("NODE_CONST %s\n", sym2name(tree));
				}
				break;
			case node_type.NODE_MATCH: {
				...
				break;
			case node_type.NODE_BACK_REF: {
					var tree = (int)tree_.cdr;
					printf("NODE_BACK_REF: $%c\n", tree);
				}
				break;
			case node_type.NODE_NTH_REF: {
					var tree = (int)tree_.cdr;
					printf("NODE_NTH_REF: $%d\n", tree);
				}
				break;
			case node_type.NODE_ARG: {
					var ast = (arg_node)tree_;
					var tree = (mrb_sym)ast.cdr;
					printf("NODE_ARG %s\n", sym2name(tree));
				}
				break;
			case node_type.NODE_BLOCK_ARG: {
					var ast = (block_arg_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_BLOCK_ARG:\n");
					parser_dump(tree, offset + 1);
				}
				break;
			case node_type.NODE_INT: {
					var ast = (int_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_INT %s base %d\n", (Uint8Array)tree.car, (int)((node)tree.cdr).car);
				}
				break;
			case node_type.NODE_FLOAT: {
					var tree = (Uint8Array)tree_.cdr;
					printf("NODE_FLOAT %s\n", tree);
				}
				break;
			case node_type.NODE_NEGATE: {
					var ast = (node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_NEGATE\n");
					parser_dump(tree, offset + 1);
			...
			case node_type.NODE_STR: {
					var ast = (str_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_STR \"%s\" len %d\n", (Uint8Array)tree.car, (int)tree.cdr);
				}
				break;
			case node_type.NODE_DSTR: {
					var ast = (dstr_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_DSTR\n");
					dump_recur((node)tree, offset + 1);
				}
			...
			case node_type.NODE_XSTR: {
					var ast = (xstr_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_XSTR \"%s\" len %d\n", (Uint8Array)tree.car, (int)tree.cdr);
				}
				break;
			case node_type.NODE_DXSTR: {
					var ast = (dxstr_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_DXSTR\n");
					dump_recur((node)tree, offset + 1);
				}
				break;
			case node_type.NODE_REGX: {
					var ast = (regx_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_REGX /%s/%s\n", (Uint8Array)tree.car, (Uint8Array)tree.cdr);
				}
				break;
			case node_type.NODE_DREGX: {
					var ast = (dregx_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_DREGX\n");
					dump_recur((node)tree.car, offset + 1);
					dump_prefix(tree, offset);
					printf("tail: %s\n", (Uint8Array)((node)((node)tree.cdr).cdr).car);
					dump_prefix(tree, offset);
					printf("opt: %s\n", (Uint8Array)((node)((node)tree.cdr).cdr).cdr);
				}
				break;
			case node_type.NODE_SYM: {
					var ast = (sym_node)tree_;
					var tree = (mrb_sym)ast.cdr;
					printf("NODE_SYM :%s\n", sym2name(tree));
				}
				break;
			case node_type.NODE_SELF: {
					var ast = (self_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_SELF\n");
				}
				break;
			case node_type.NODE_NIL: {
					var ast = (nil_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_NIL\n");
				}
				break;
			case node_type.NODE_TRUE: {
					var ast = (true_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_TRUE\n");
				}
				break;
			case node_type.NODE_FALSE: {
					var ast = (false_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_FALSE\n");
				}
				break;
			case node_type.NODE_ALIAS: {
					var ast = (alias_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_ALIAS %s %s:\n",
					sym2name((mrb_sym)tree.car),
					sym2name((mrb_sym)tree.cdr));
				}
				break;
			case node_type.NODE_UNDEF: {
					var ast = (undef_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_UNDEF");
					{
						node t = tree;
						while (t != null) {
							printf(" %s", sym2name((mrb_sym)t.car));
							t = (node)t.cdr;
						}
					}
					printf(":\n");
				...
				break;
			case node_type.NODE_CLASS: {
					var ast = (class_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_CLASS:\n");
					if ((MrbTokens)((node)tree.car).car == (MrbTokens)0) {
						dump_prefix(tree, offset + 1);
						printf(":%s\n", sym2name((mrb_sym)((node)tree.car).cdr));
					}
					else if ((MrbTokens)((node)tree.car).car == (MrbTokens)1) {
						dump_prefix(tree, offset + 1);
						printf("::%s\n", sym2name((mrb_sym)((node)tree.car).cdr));
					}
					else {
						parser_dump((node)((node)tree.car).car, offset + 1);
						dump_prefix(tree, offset + 1);
						printf("::%s\n", sym2name((mrb_sym)((node)tree.car).cdr));
					}
					if (((node)tree.cdr).car != null) {
						dump_prefix(tree, offset + 1);
						printf("super:\n");
						parser_dump((node)((node)tree.cdr).car, offset + 2);
					}
					dump_prefix(tree, offset + 1);
					printf("body:\n");
					parser_dump((node)((node)((node)((node)tree.cdr).cdr).car).cdr, offset + 2);
				}
				break;
			case node_type.NODE_MODULE: {
					var ast = (module_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_MODULE:\n");
					if ((MrbTokens)((node)tree.car).car == (MrbTokens)0) {
						dump_prefix(tree, offset + 1);
					...
					dump_prefix(tree, offset + 1);
					printf("body:\n");
					parser_dump((node)((node)((node)tree.cdr).car).cdr, offset + 2);
				}
				break;
			case node_type.NODE_SCLASS: {
					var ast = (sclass_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_SCLASS:\n");
					parser_dump((node)tree.car, offset + 1);
					dump_prefix(tree, offset + 1);
				...
				break;
			case node_type.NODE_DEF: {
					var ast = (def_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_DEF:\n");
					dump_prefix(tree, offset + 1);
					printf("%s\n", sym2name((mrb_sym)tree.car));
					...
					if (tree.car != null) {
						...
						if (n.cdr != null) {
							dump_prefix(n, offset + 1);
							printf("blk=&%s\n", sym2name((mrb_sym)n.cdr));
						}
					}
					parser_dump((node)((node)tree.cdr).car, offset + 1);
				}
				break;
			case node_type.NODE_SDEF: {
					var ast = (sdef_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_SDEF:\n");
					parser_dump((node)tree.car, offset + 1);
					tree = (node)tree.cdr;
				...
				break;
			case node_type.NODE_POSTEXE: {
					var ast = (postexe_node)tree_;
					var tree = (node)ast.cdr;
					printf("NODE_POSTEXE:\n");
					parser_dump(tree, offset + 1);
				}
			...
			case node_type.NODE_HEREDOC: {
					var tree = (parser_heredoc_info)tree_.cdr;
					printf("NODE_HEREDOC:\n");
					parser_dump(((parser_heredoc_info)tree).doc, offset + 1);
				}
				break;
			default: {
