...
{
  if (a != 0) {
    if (a < mrb_preset_symbols_count)
      return 0;
    a -= mrb_preset_symbols_count;
  }
  if (b != 0) {
    if (b < mrb_preset_symbols_count)
      return 0;
    b -= mrb_preset_symbols_count;
  }
  return (mrb->symtbl[a].len == mrb->symtbl[b].len && memcmp(mrb->symtbl[a].name, mrb->symtbl[b].name, mrb->symtbl[a].len) == 0);
}
#else
...
sym_intern(mrb_state *mrb, const char *name, size_t len, mrb_bool lit)
{
  ...
  sym = ++mrb->symidx;
#ifdef MRB_USE_PRESET_SYMBOLS
  if (mrb->symcapa < (sym - mrb_preset_symbols_count)) {
    mrb->symcapa = (size_t)(mrb->symcapa + 100);
    mrb->symtbl = (symbol_name*)mrb_realloc(mrb, mrb->symtbl, sizeof(symbol_name)*(mrb->symcapa+1));
  }
  sname = &mrb->symtbl[sym - mrb_preset_symbols_count];
#else
  if (mrb->symcapa < sym) {
...
#ifdef MRB_USE_PRESET_SYMBOLS
  if (sym <= mrb_preset_symbols_count) {
    return mrb_preset_sym2name(sym, lenp);
  }

  if (lenp) *lenp = mrb->symtbl[sym - mrb_preset_symbols_count].len;
  return mrb->symtbl[sym - mrb_preset_symbols_count].name;
#else
...
#ifdef MRB_USE_PRESET_SYMBOLS
  for (i=1, lim=mrb->symidx-mrb_preset_symbols_count+1; i<lim; i++) {
    if (!mrb->symtbl[i].lit) {
